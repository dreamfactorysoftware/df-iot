...
const telemetry = require('./endpoints').telemetry
...
function build (opts, logger, cb) {
  ...
  server.published = function (packet, client, callback) {
    ...
    var path = telemetry.path

    if (!parsed.value.timeseries) {
      logger.debug({ deviceId, payload: parsed.value }, 'uploading to messages, as timeseries field is missing')
      path = messages.path
    }
    ...
    for (var key in parsed.value) {
      if (key !== 'timeseries') {
        toUpload[key] = parsed.value[key]
      }
    }

    logger.debug({ deviceId, payload: toUpload }, 'uploading to telemetry')
